...
def aruco_detection_publisher():
    ...
    try:
        ...
        while not rospy.is_shutdown():
            ...
            h, w = aruco_img.shape[:2]
            # print(h, w)
            newcameramtx, roi = cv2.getOptimalNewCameraMatrix(k, d, (w, h), 1, (w, h))
            # print(newcameramtx)
            # print(k)
            dst = cv2.undistort(aruco_img, k, d, None, newcameramtx)
            x, y, w, h = roi
            dst = dst[y:y+h, x:x+w]
            aruco_frame = dst
            # aruco_frame_ycrcb = cv2.cvtColor(aruco_frame, cv2.COLOR_BGR2YCrCb)
            # ycrcb_planes = cv2.split(aruco_frame_ycrcb)

            # ycrcb_planes[0] = cv2.equalizeHist(ycrcb_planes[0])

            # dst_ycrcb = cv2.merge(ycrcb_planes)
            # aruco_dst = cv2.cvtColor(dst_ycrcb, cv2.COLOR_YCrCb2BGR)

            # cv2.imshow('src', aruco_frame)
            # cv2.imshow('dst', aruco_dst)
            # cv2.waitKey(1)

            mid_x, mid_y = w//2, h//2
            # print(mid_x, mid_y)
            
            img_frame, rotation, translation, ids = pose_esitmation(aruco_frame, aruco_dict_type, k, d, mid_x, mid_y)
            ...
            imgage_msg = bridge.cv2_to_imgmsg(img_frame, "bgr8")
